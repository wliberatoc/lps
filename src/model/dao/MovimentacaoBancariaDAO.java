...
public class MovimentacaoBancariaDAO {
    ...
    public boolean insert(MovimentacaoBancaria moviB){
        String sql = "INSERT INTO tbl_movimentacao_bancaria (id, id_movimentacao_bancaria, data, valor, id_tipo_operacao, descricao, tipo_movimentacao) VALUES (?, ?, ?, ?, ?, ?)" ;
        try {
    ...
    public ArrayList<MovimentacaoBancaria> load(int id){
        String sql = "SELECT * FROM tbl_movimentacao_bancaria WHERE tbl_movimentacao_bancaria.id = ?";
        ArrayList<MovimentacaoBancaria> lista  = new ArrayList<>();
        try {
            stmt = Persistencia.getConnection().prepareStatement(sql);
            stmt.setInt(1, id);
